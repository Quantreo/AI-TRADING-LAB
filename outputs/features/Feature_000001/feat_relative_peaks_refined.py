...
def relative_peaks_proportion(df: pd.DataFrame, high_col: str = 'high', peak_multiplier: float = 1.2, window_size: int = 5) -> pd.Series:
    """
    ...
    result = pd.Series([adjusted_proportion]).rolling(window=window_size).mean()
